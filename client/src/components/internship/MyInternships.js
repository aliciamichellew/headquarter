import { Box, Button, createTheme, Grid, Pagination, Stack, styled, TextField, ThemeProvider, Typography, useTheme } from "@mui/material";
...
export default function MyInternship() {
  ...
  useEffect(() => {
    const fetchMyInternship = async () => {
      ...
      setLoading(true);
      const res = await axios.get(`/api/internships/myinternship/${userId}`, {userId}, config);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `/api/internships/myinternship/search/${userId}/${searchQuery}`, {userId},
        { searchQuery },
        config
      );
